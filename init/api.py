...
def setup_api(default_mod, config_mod):
    ...
    pkg_files = [
        ...
        f"api/app/src/test/kotlin/{def_pkg_path}/ArchitectureTest.kt",
        f"api/app/src/main/kotlin/{def_pkg_path}/{def_pascal}Application.kt"
    ]
    for f in pkg_files:
        replace_text(f, def_pkg, cfg_pkg)

    # Replace image and name
    replace_text("api/compose.prod.yml", def_img, cfg_img)
    replace_text("api/app/src/main/resources/application.yml", def_name, cfg_name)

    # Replace pascal case name
    app_file = f"api/app/src/main/kotlin/{def_pkg_path}/{def_pascal}Application.kt"
    ...
    # Move main package
    move_files(
        f"api/app/src/main/kotlin/{def_pkg_path}",
        f"api/app/src/main/kotlin/{cfg_pkg_path}"
    )

    # Move test package
    move_files(
        f"api/app/src/test/kotlin/{def_pkg_path}",
        f"api/app/src/test/kotlin/{cfg_pkg_path}"
    )
    ...
    move_files(
        f"api/app/src/main/kotlin/{cfg_pkg_path}/{def_pascal}Application.kt",
        f"api/app/src/main/kotlin/{cfg_pkg_path}/{cfg_pascal}Application.kt"
    )

def delete_api():
    delete_files("api")
    # Delete GitHub workflows
    for f in glob.glob(".github/workflows/api-*.yml"):
        delete_files(f)
    remove_dependabot_ecosystem("/api")
