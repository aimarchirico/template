...
A monorepo for the template npm containing the Expo host application, client API library, and shared configurations.
...
The module utilizes a `.env` file at the `npm` root for local environment variables:
...
Install workspace dependencies from the `npm` root:
...
- **Android deployment** (`.github/workflows/npm-android.yml`): Triggers on `npm-v*` tags, builds the release APK via `task ci:android`, and uploads the release asset.
- **Web deployment** (`.github/workflows/npm-web.yml`): Triggers on `npm-v*` tags, exports the web application, and deploys the static files to Cloudflare Pages.
