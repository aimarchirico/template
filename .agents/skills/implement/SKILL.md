...
7. Execute the `pr` skill to open a pull request, passing the `<issue-id>` to link the issue. Pass the --draft flag to the `pr` skill if it was provided by the user.
